...
def process(dataset, index_start_from=0):
    ...
    tf_writer = HeatmapRecordOperator(
    ...
    try:
        ...
        for sample in tqdm(dataset):
            ...
            image_translated, trans_vector = move_face_to_center(image,
                                                                 marks, mo)
            marks_translated = marks[:, :2] + trans_vector

            # Second, align the face. This happens in the 2D space. This is not
            # the case for our HRNet experiment. But I will keep these lines
            # here in case you need them.
            image_rotated, degrees = rotate_to_vertical(image_translated,
                                                        sample, mo)
            img_height, img_width, _ = image.shape
            marks_rotated = mo.rotate(marks_translated, degrees/180*np.pi,
                                      (img_width/2, img_height/2))

            # Third, try to crop the face area out. Pad the image if necessary.
            image_cropped, padding, bbox = crop_face(image_translated, marks_translated,
                                                     scale=1.5)
            mark_cropped = marks_translated + padding - \
                np.array([bbox[0], bbox[1]])

            # Last, resize the face area.
            image_resized = cv2.resize(image_cropped, (256, 256))
            mark_resized = mark_cropped * (256 / image_cropped.shape[0])
            ...
            # Generate face mark heatmaps.
            mark_normalized = mark_cropped / image_cropped.shape[0]
            heatmaps = mo.generate_heatmaps(mark_normalized, (64, 64), 3)
            ...
            # # Preview the image?
            # heatmap_merged = np.sum(heatmaps, axis=0)
            # cv2.imshow("Heatmap", cv2.resize(heatmaps[0], (512, 512)))
            # if cv2.waitKey() == 27:
            #     break
...
def move_face_to_center(image, marks, mo):
    ...
    image_translated = cv2.warpAffine(image, translation_mat,
                                      (img_width, img_height))

    translation_vector = np.array([img_width / 2 - face_center[0],
                                   img_height / 2 - face_center[1]])
...
def crop_face(image, marks, scale=1.8, shift_ratios=(0, 0)):
    ...
    Returns:
        cropped face image.
    """
    # In case the input marks are 2 dimensional, expand it to 3D.
    if marks.shape[1] == 2:
        marks = np.concatenate((marks, np.zeros([marks.shape[0], 1])), axis=-1)
...
def show_debug_images(image_translated, marks_translated, image_rotated,
                      marks_rotated, image_resized, mark_resized, padding, bbox):
...
if __name__ == "__main__":
    ...
    # WFLW
    ds_wflw = fmd.wflw.WFLW("wflw_train")
